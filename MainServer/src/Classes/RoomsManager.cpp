...
namespace Main
{
    namespace Classes
    {
        ...
        std::vector<Main::Network::Session*> RoomsManager::getSessions(std::uint16_t roomNumber)
        {
            if (m_roomByNumber.contains(roomNumber))
            {
                return m_roomByNumber[roomNumber].getSessions(); // This returns std::vector<Main::Network::Session*>
            }
            return {}; // Return an empty vector if the room doesn't exist
        }
