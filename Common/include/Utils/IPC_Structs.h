...
namespace Utils
{
    ...
    public:
        ...
        static std::uint64_t getCurrentTimeInMilliseconds()
        {
            using namespace std::chrono;
            auto durationSinceEpoch = system_clock::now().time_since_epoch();
            return static_cast<std::uint64_t>(duration_cast<milliseconds>(durationSinceEpoch).count());
        }
    };
}
